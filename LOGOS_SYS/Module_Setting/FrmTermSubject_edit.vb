...
Public Class FrmTermSubject_edit
    ...
    Private Sub load_data()
        ...
        Sql &= " WHERE(term_subject_id=" & id_edit & ") "
        dt = ExecuteDatable(Sql)
        With dt.Rows(0)
            txt_subjecct.Text = .Item("subject_name_la")
            txt_subject_id.Text = .Item("subject_id")
            txt_remark.Text = .Item("more_des")
            cb_course.SelectedValue = .Item("course_id")
            Dim term_id As Integer = .Item("term_id")

            Call getTermList(cb_course.SelectedValue)
            cb_term.SelectedValue = term_id
        End With
    End Sub
    ...
    Private Sub edit_user()
        ...
        Call ConnectDB()
        Sql = "UPDATE tbl_setting_term_subject SET term_id=@term_id ,subject_id=@subject_id ,more_des=@more_des ,user_update=@user_update, last_update=getdate() "
        Sql &= " WHERE(term_subject_id=@id_edit)"
