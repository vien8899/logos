Public Class Frm_Report_Enroll
    ...
    Public Sub LoadData()
        ...
        If (cb_user.SelectedValue <> 0) Then
            ct_user = " AND (user_id = " & cb_user.SelectedValue & ")"
        End If

        Sql = "SELECT * FROM view_std_enroll "
        Sql &= " WHERE(enroll_date BETWEEN '" & s_d & "' AND '" & e_d & "')" & ct_payment & ct_course & ct_user
        Sql &= " ORDER BY scheme_id, course_id, enroll_id "
        dt = ExecuteDatable(Sql)
        Dim crt As New Rpt_Enroll
